...
const viewAllAvailableItems = (request, response) => {
  pool.query(
    'SELECT * FROM Items I LEFT OUTER JOIN Transactions T on I.itemSSN = T.itemSSN where T.transactionSSN = NULL OR NOT EXISTS (select 1 WHERE T.returnedStatus = FALSE)',
    (error, results) => {
      if (error) {
        throw error;
      }
...
const searchAllItems = (request, response) => {
  ...
  pool.query(
    'SELECT * FROM Items WHERE loanedByUserSSN = $1',
    [loanedByUserSSN],
    (error, results) => {
      if (error) {
        throw error;
...
const deleteBid = (request, response) => {
  ...
  pool.query('DELETE FROM Bids WHERE bidSSN = $1', [bidSSN], (error) => {
    if (error) {
      throw error;
    }
...
const updateBid = (request, response) => {
  ...
  pool.query('UPDATE Bid SET bidAmt = ${bidAmt} WHERE bidSSN = ${bidSSN}', (error) => {
    if (error) {
      throw error;
    }
...
const viewAllItemBid = (request, response) => {
  ...
  pool.query('SELECT * FROM Bids WHERE itemSSN = $1', [itemSSN], (error, results) => {
    if (error) {
      throw error;
    }
    response.status(200).json(results.rows);
...
const viewWinningBid = (request, response) => {
  ...
  pool.query('SELECT * FROM WinningBids WHERE itemSSN = $1', [itemSSN], (error, results) => {
    if (error) {
      throw error;
    }
    response.status(200).json(results.rows);
